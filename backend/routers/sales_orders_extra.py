...
async def list_orders(request: Request, status: str = None, customer_id: str = None,
    ...
    ctx = await entity_ctx(request)
    # REVISI SAMPEL — daftar SO biasa TIDAK memuat pesanan sampel (SMP-); ada daftarnya sendiri.
    query = {"order_type": {"$ne": "sample"}}
    if status:
...
async def get_orders_stats(request: Request, entity_id: str = None,
    ...
    ctx = await entity_ctx(request)
    scope = resolve_list_scope("sales_orders", {"order_type": {"$ne": "sample"}}, ctx, entity_id)
